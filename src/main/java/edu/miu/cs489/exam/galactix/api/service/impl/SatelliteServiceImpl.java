...
public class SatelliteServiceImpl implements SatelliteService {
    ...
    public SatelliteResponseDTO updateSatellite(Long id, SatelliteRequestDTO satelliteRequestDTO) {
        ...
        if(satelliteRepository.existsByName(satelliteRequestDTO.name())) {
            throw new DuplicateSatelliteException(
                    String.format("Satellite with name %s already exists", satelliteRequestDTO.name())
            );
        }
